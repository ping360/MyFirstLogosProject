...
public class DigitalUnitController {
	...
	public String save(@ModelAttribute("du")@Valid DigitalUnit form, BindingResult br, Model model, SessionStatus status, @PageableDefault Pageable pageable, @ModelAttribute("filter") BasicFilter filter){
		if(br.hasErrors()){
			show(model, pageable, filter);
			return "admin-digitalUnit";
		}
